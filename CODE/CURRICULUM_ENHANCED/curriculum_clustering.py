...
def cluster_curriculum_subsets(X, y, n_subsets=3, method='default', density_t=0.6, verbose=False,
    ...
    pca = None
    auxiliary_info = []
    if X.shape[1] > dim_reduce:     # 降维
        pca = PCA(n_components=dim_reduce, copy=False, random_state=random_state)

    all_clustered_labels = np.full(len(y), -1, dtype=np.intp)   # 聚类标签

    for cluster_idx, current_category in enumerate(unique_categories):      # 以类别做循环查找
        if verbose:
            t0 = time.time()
        ...
        for batch_range in gen_batches(len(dist_list), batch_size=batch_max):
            batch_dist_list = dist_list[batch_range]    # 对该类重新索引，形成数组

            # Load data subset
            subset_vectors = np.zeros((len(batch_dist_list), X.shape[1]), dtype=np.float32) # [该类个数，特征维数]
            ...
            if method == 'gaussian':
                ...
                densities = np.zeros((len(subset_vectors)), dtype=np.float32) # [该类个数]
                # densities2 = np.zeros((len(subset_vectors)), dtype=np.float32) # [该类个数]

                flat_distance = distance.reshape(distance.shape[0] * distance.shape[1])
                dist_cutoff = np.sort(flat_distance)[int(distance.shape[0] * distance.shape[1] * density_t)] # 从小到大排列，第60%个元素对应的距离
                for i in range(len(batch_dist_list)):
                    pos = np.where(distance[i] < dist_cutoff) # 逐个元素都有比较，把60%以前的对应索引号拿出来
                    densities[i] = len(pos[0]) - 1  # remove itself   密度，指的是满足上一个条件的，所有元素，个数 （不包含自己）
            if len(densities) < n_subsets:      # 本身个数不多，密度排布也比较散，这种情况就属于复杂样本了（个数为2时）
                for i in range(len(batch_dist_list)):
                    c = batch_dist_list[i]    # batch_dist_list指的是ID对应第哪些张图片
                    all_clustered_labels[c] = 3        # 个数为1时，也视为复杂样本
                break
            ...
            if calc_auxiliary:
                ...
                for i in range(len(densities_sort_idx) - 1):
                    ...
                    larger = larger[np.where(larger != densities_sort_idx[i])]
            ...
            model.fit(densities.reshape(len(densities), 1)) # 聚类算法，找到三类分别的种子点（根据密度聚类）

            clusters = [densities[np.where(model.labels_ == i)] for i in range(n_subsets)] # 根据index分成三类（list）
            # clusters内容为：分了三组的密度数。densities则是未分组前的

            # n_clusters_made = len(set([k for j in clusters for k in j]))    # 计算可聚类多少组数

            length = set([k for j in clusters for k in j])  # 不重复显示密度

            n_clusters_made = len(length)

            if n_clusters_made < n_subsets:     # 不重复的密度数小于需分的组数，类内数量等于3时（减去本身，只和两类比较了）
                # 这种情况下，分为简单组
                for i in range(len(batch_dist_list)):
                    c = batch_dist_list[i]    # 少的也是难的
                    all_clustered_labels[c] = 3
                break

            cluster_mins = [np.min(c) for c in clusters]    # 每个聚类中最小密度数 list
            bound = np.sort(np.array(cluster_mins))     # 转换成ndarray类型
            ...
            for i in range(len(densities)):        # 复杂=2 简单=0
                if densities[i] >= bound[n_subsets - 1]: # 简单组
                    a = batch_dist_list[i]
                    all_clustered_labels[a] = 0
                else:
                    for j in other_bounds:
                        if bound[j] <= densities[i] < bound[j + 1]:
                            b = batch_dist_list[i]
                            all_clustered_labels[b] = len(bound) - j - 1
    ...
    return all_clustered_labels, auxiliary_info     # 第一个返回值：聚类标签
